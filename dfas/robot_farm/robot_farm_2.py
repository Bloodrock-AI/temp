...
G1  = Node("G1")                 # unlocked
G2  = Node("G2")                 # at A
G3  = Node("G3")                 # watered A
G4  = Node("G4")                 # at C
G5  = Node("G5")                 # watered C
G6  = Node("G6")                 # at water station
G7  = Node("G7")                 # refilled
G8  = Node("G8")                 # at B
G9  = Node("G9")                 # watered B
G10 = Node("G10", is_final=True) # home
...
alphabet: dict[str, FunctionCall] = {
    ...
    "C'''''": FunctionCall(
        name="move_to",
        arguments={
            "x":   FunctionArgument(name="x",   value=None, type="float", excluded_values=[2.0, 14.0, 18.5, 3.5, 5.0]),
            "y":   FunctionArgument(name="y",   value=None, type="float", excluded_values=[14.0, 8.5, 2.0, 12.5, 5.0]),
    ...
    "G": FunctionCall(
        name="water_plant",
        arguments={
            "plant_id": FunctionArgument(name="plant_id", value="plant_A", type="str",   excluded_values=None),
            "liters":   FunctionArgument(name="liters",   value=1.5,       type="float", excluded_values=None),
        },
    ),
    ...
    "G'''": FunctionCall(
        name="water_plant",
        arguments={
            "plant_id": FunctionArgument(name="plant_id", value=None, type="str",   excluded_values=["plant_A", "plant_C", "plant_B"]),
            "liters":   FunctionArgument(name="liters",   value=None, type="float", excluded_values=[1.5, 2.5]),
        },
    ),
...
G6.transitions = [
    Transition(symbols=["I"], _from=G6, _to=G7),
    Transition(symbols=READS, _from=G6, _to=G6),
]

G7.transitions = [
    Transition(symbols=["C'''"], _from=G7, _to=G8),
...
G9.transitions = [
    Transition(symbols=["D", "C''''"], _from=G9, _to=G10),
