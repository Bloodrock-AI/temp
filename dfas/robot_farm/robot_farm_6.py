# robot_farm_6.py
# DFA for:
# "Harvest plant A, then plant C. Take each load to the collection bin, empty the hopper, and return to base."
#
# Golden path (matches expected_sequences):
#   unlock -> move_to(A) -> harvest(A) -> move_to(C) -> harvest(C) ->
#   move_to(bin) -> dump_hopper -> move_home
...
G0 = Node("G0")                 # locked
G1 = Node("G1")                 # unlocked
G2 = Node("G2")                 # at plant_A
G3 = Node("G3")                 # harvested A
G4 = Node("G4")                 # at plant_C
G5 = Node("G5")                 # harvested C
G6 = Node("G6")                 # at collection_bin
G7 = Node("G7")                 # dumped
G8 = Node("G8", is_final=True)  # home
...
alphabet: dict[str, FunctionCall] = {
    ...
    "C''''": FunctionCall(
        name="move_to",
        arguments={
            "x":   FunctionArgument(name="x",   value=None, type="float", excluded_values=[2.0, 14.0, 6.0, 5.0]),
            "y":   FunctionArgument(name="y",   value=None, type="float", excluded_values=[14.0, 8.5, 18.0, 5.0]),
