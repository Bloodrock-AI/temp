...
alphabet: dict[str, FunctionCall] = {
    ...
    "G": FunctionCall(
        ...
        arguments={
            "account_id": FunctionArgument(name="account_id", value="A123", excluded_values=None, type="str"),
            "rate": FunctionArgument(name="rate", value=0.05, excluded_values=None, type="float"),
        },
    ),
    "G'": FunctionCall(
        name="apply_interest",
        arguments={
            "account_id": FunctionArgument(name="account_id", value=None, excluded_values=["A123"], type="str"),
            "rate": FunctionArgument(name="rate", value=None, excluded_values=[0.05], type="float"),
        },
    ),
    "H": FunctionCall(
        name="close_account",
...
G0.transitions = [
    ...
    Transition(symbols=["D", "D'", "F"], _from=G0, _to=G0),
]

G1.transitions = [
    Transition(symbols=["B"], _from=G1, _to=G2),
    Transition(symbols=["D", "D'", "F"], _from=G1, _to=G1),
]

G2.transitions = [
    Transition(symbols=["G"], _from=G2, _to=G3),
    Transition(symbols=["D", "D'", "F"], _from=G2, _to=G2),
]
